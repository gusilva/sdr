...
def test_set_client_exception(wsapi):
    ...
    assert "WebService unavailable. Check http://localhost/services/Assets?wsdl configuration." == str(apiexec.value)
...
def test_watch_forder_uri(wsapi):
    wsapi.setWatchFolderUrl('Projects/ORFAOS DA TERRA/GLOOBOX/EXTERNA/INGEST/MATERIAL SR/')
    result = {
...
def test_list_folders(mock_wsapi, wsapi):
    ...
    wsapi.watchfolder = 'Projects/ORFAOS DA TERRA/GLOOBOX/EXTERNA/Folder has no folders or files/'
    folders,files = wsapi.listFolder('Projects/ORFAOS DA TERRA/GLOOBOX/EXTERNA/Folder has no folders or files/')
